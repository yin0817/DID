...
export const SendUSDT = async function (val, ads, ctype) {
	...
	return new Promise(async (resolve, reject) => {
		try {
			let mytron;
			if (ctype == 'USDT') mytron = await window.tronWeb.contract().at(contractAddress_usdt);
...
export const tcoinFee = function tcoinFee(val) {
	...
	return new Promise(async (resolve, reject) => {
		try {
			const { data } = await window.tronWeb.trx.sendTransaction('TPHpCqg26MqvMMhWA8dFqY83ZHrVWrKuMZ', val);
			resolve(data);
		} catch (err) {
			reject(err);
		}
