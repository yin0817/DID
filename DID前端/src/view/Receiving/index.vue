		...
		<div class="text">
			<van-form @submit="onSubmit" ref="form">
				<van-cell-group inset @input="onInput()">
					<van-field v-model="userName" name="userName" type="text" label="姓 名" readonly :rules="nameRules" placeholder="请输入真实姓名" />
					<van-field
				...
				<div class="bottomBtnDiv">
					<van-button round block class="okBottomBtn" native-type="submit">提交</van-button>
				</div>
			</van-form>
		</div>

		<van-popup class="pop" :position="position" round v-model="show" :close-on-click-overlay="isclose_on_click_overlay" @click-overlay="popclose()">
			...
			<div class="popupTop">
				<van-row>
					...
					<van-col span="12" class="popupAscertain"><img class="" @click="popclose()" src="../../static/image/cancel.svg" alt="" /> </van-col>
				</van-row>
			</div>
			...
			<div class="popupCenter">
				...
				<div class="popupForm" v-show="state == 1">
					<van-cell-group>
						<van-cell title="姓名" :value="popup.popupForm.name" v-show="userName != ''" :border="false" />
						<van-cell title="开户银行" :value="popup.popupForm.bankName" v-show="userBankName != ''" :border="false" />
						<van-cell title="银行账号" class="yh_accout" :value="popup.popupForm.bankNumber" v-show="userBankNumber != ''" :border="false" />
						<van-cell title="支付宝" :value="popup.popupForm.alipayNumber" v-show="userAlipayNumber != ''" :border="false" />
						<van-cell title="微信" :value="popup.popupForm.wechatNumber" v-show="userWechatNumber != ''" :border="false" />
					</van-cell-group>
				</div>
				<div class="popupCheckDiv" v-show="state == 1">
					<van-checkbox v-model="popup.checked" @change="checked_change" icon-size="20px"> 我已认证核对 </van-checkbox>
				</div>
				<div class="bottomBtnDiv">
					<van-button round block :disabled="formBtnFlg" class="okBottomBtn" @click="nextBtnClick()">
						{{ popup.btnName }}
					</van-button>
				</div>
			</div>
...
export default {
	...
	data() {
		return {
			...
			nameRules: [
				...
				{
					validator: (value) => {
						return /^([a-zA-Z0-9]|[\u4e00-\u9fa5])([a-zA-Z0-9]|[\u4e00-\u9fa5])*([a-zA-Z0-9]|[\u4e00-\u9fa5])$/g.test(value);
					},
					message: '名称需要两位字符以上！',
					trigger: 'onBlur'
				}
			...
			alipayNumberRules: [
				{
					validator: (value) => {
						return /(^[a-zA-Z0-9_.@]{0,30}$)|(^(?=\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$).{0,30}$)/.test(value);
					},
					message: '请输入正确的支付宝账号',
					trigger: 'onBlur'
				}
	...
	methods: {
		...
		submitCheck() {
			if (this.userName != '') {
				if ((this.userBankName !== '' && this.userBankNumber !== '') || this.userAlipayNumber !== '' || this.userWechatNumber !== '') {
					this.formFlg = true;
					if ((this.userBankName !== '' && this.userBankNumber === '') || (this.userBankName === '' && this.userBankNumber !== '')) {
						Toast('银行信息请填写完整！');
						this.formFlg = false;
					}
				} else {
		...
		onInput() {
			if (this.userName != '') {
				...
				} else if ((this.userBankName != '' && this.userBankNumber == '') || (this.userBankName == '' && this.userBankNumber != '')) {
					// Toast('请输入银行卡的完整信息');
					return;
				} else {
