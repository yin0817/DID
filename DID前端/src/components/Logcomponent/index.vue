<template>
	<div>
      ...
      >
        <template #reference> -->
			<p class="hint">当前网络</p>
			<van-field class="cell" v-model="value1" readonly arrow-direction="down" />
			<!-- </template>
      </van-popover> -->
			<p class="hint">钱包地址</p>
			<van-field readonly class="cell" clearable v-model="wallet" />
			<p class="hint">输入邮箱地址</p>
			<van-field class="cell" v-model="email" center clearable @focus="focus()" @blur="blur()">
				<template #button>
					<van-button size="small" v-show="!countShow" class="cell_button" :class="{ button_action: buttonShow && !countShow }" :disabled="!buttonShow" @click="gain">
						<span>发送验证码</span>
					</van-button>
		...
		<div class="footer">
			<van-button round block :disabled="value1 != '' && email != '' && verification != '' && wallet != '' ? false : true" color="#1B2945" @click="login">登录</van-button>
			<p>
				还没有账户
